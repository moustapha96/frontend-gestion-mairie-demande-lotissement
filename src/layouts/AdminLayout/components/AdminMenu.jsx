...
// export default function AdminMenu() {
//   const { pathname } = useLocation();
//   const { user } = useAuthContext(); // user.roles : string[]
//   const userRoles = Array.isArray(user?.roles) ? user.roles : [];
...
//       {/* Header (logo + burger) */}
//       <div className="container mx-auto flex items-center justify-between px-3 py-2.5">
//         <Link to="/admin/dashboard" className="font-bold text-default-800">Admin</Link>
...
//       {/* Rangée top-level */}
//       <div className="border-t border-default-200 md:border-t-0">
//         <div className={cn("container mx-auto px-3", mobileOpen ? "block" : "hidden md:block")}>
//           <div className="flex items-center gap-1 overflow-x-auto custom-scrollbar py-2.5">
//             {items.map((item, idx) => {
//               const Icon = item.icon;
//               const active = isActivePath(pathname, item);
...
//                     {Icon && <Icon className="size-5" />}
//                     <span>{item.name}</span>
//                     <LuChevronDown className={cn("size-4 transition-transform", isOpen && "rotate-180")} />
//                   </button>
//                 );
//               }
