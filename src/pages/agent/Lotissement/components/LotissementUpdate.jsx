...
const AgentLotissementModifier = ({ isEditing = false }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const body = { nom, localisation, description, statut, dateCreation: dateCreation.replace("T", " ") };
            await updateLotissement(id, body);
            toast.success("Lotissement mis à jour avec succès!");
            navigate("/agent/lotissements");
        } catch (error) {
            console.error(error);
    ...
    return (
        <>
            <AgentBreadcrumb title={isEditing ? "Mise à jour du lotissement" : "Nouveau lotissement"} />
                ...
                <div className="bg-white shadow-lg rounded-lg overflow-hidden w-full max-w-2xl p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {/* <div>
                            ...
                            <input type="datetime-local" id="dateCreation" value={dateCreation} onChange={(e) => setDateCreation(e.target.value)} required
                                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50" />
                        </div> */}
                        <div className="flex justify-center">
...
export default AgentLotissementModifier;
