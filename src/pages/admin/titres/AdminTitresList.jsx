...
import { Table, Card, Button, Form, Input, Select, message, Popconfirm, Space } from "antd";
...
const fileUrl = (p) => {
  ...
  const columns = [
    ...
    { title: "Superficie (m²)", dataIndex: "superficie", width: 160 },
    { title: "Quartier", dataIndex: ["quartier", "nom"], render: (_, r) => r.quartier?.nom || "—" },
    {
      title: "Fichier",
      dataIndex: "fichier", // Utilise "fichier" au lieu de "fichierUrl"
      width: 140,
      render: (url, r) => {
        const filePath = r.fichier;
        return filePath ? (
          <Button
            size="small"
            icon={<LinkOutlined />}
            onClick={() => window.open(fileUrl(filePath), "_blank")}
          >
            Ouvrir
          </Button>
        ) : "—";
