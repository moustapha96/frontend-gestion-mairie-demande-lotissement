...
const toCSV = (rows = []) => {
  const headers = [
    "id","dateAction","action",
    "demandeId","typeDemande","typeDocument","statut","superficie",
    "validateurId","validateurNom","validateurPrenom","validateurEmail",
    "niveauOrdre","niveauNom","roleRequis","statutAvant","statutApres",
    "commentaire","motif"
  ];
  const escapeCSV = (v) => {
    if (v === null || v === undefined) return "";
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, r) => (
        <Space direction="vertical" size={0}>
          <Space wrap>
            {typeof r.niveauOrdre === "number" && <Tag>Ordre: {r.niveauOrdre}</Tag>}
            {r.niveauNom && <Tag color="blue">{r.niveauNom}</Tag>}
          </Space>
          {r.roleRequis && (
            <Space wrap>
              <Tag color="geekblue">{r.roleRequis}</Tag>
            </Space>
          )}
        </Space>
      ),
    ...
    {
      title: "Transition de statut",
      key: "statuts",
      width: 220,
      render: (_, r) => (
        <Space>
          {statutTag(r.statutAvant)}
          <span style={{ color: "#999" }}>→</span>
          {statutTag(r.statutApres)}
        </Space>
      ),
    },
    {
            ...
            <Card>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                <Space>
                  <Button icon={<DownloadOutlined />} onClick={exportCSV}>Export CSV</Button>
                  <span>Afficher en modal</span>
                  <Switch checked={useModal} onChange={setUseModal} />
                </Space>
              </div>
              ...
              >
                ...
                <Form.Item label="Niveau (ordre)">
                  <Input
                    type="number"
                    min={1}
                    style={{ width: 110 }}
                    onChange={(e) => setFilters(s => ({ ...s, niveauOrdre: e.target.value ? Number(e.target.value) : null }))}
                  />
                </Form.Item>
                <Form.Item label="Niveau (nom)">
                  <Input
                    placeholder="ex: AGENT"
                    onChange={(e) => setFilters(s => ({ ...s, niveauNom: e.target.value || null }))}
                  />
                </Form.Item>
                <Form.Item label="Rôle requis">
                  <Input
                    placeholder="ex: ROLE_PRESIDENT_COMMISSION"
                    onChange={(e) => setFilters(s => ({ ...s, roleRequis: e.target.value || null }))}
                  />
                </Form.Item>
                {/* <Form.Item label="Statut avant">
                  <Select
                    allowClear style={{ width: 200 }}
                    onChange={(v) => setFilters(s => ({ ...s, statutAvant: v || null }))}
                    options={[
                ...
                {/* <Form.Item label="Statut après">
                  <Select
                    allowClear style={{ width: 200 }}
                    onChange={(v) => setFilters(s => ({ ...s, statutApres: v || null }))}
                    options={[
                      { value: "En attente", label: "En attente" },
                      { value: "En cours de traitement", label: "En cours de traitement" },
                      { value: "Rejetée", label: "Rejetée" },
                ...
                expandable={{
                  expandedRowRender: (r) => {
                    ...
                    return (
                      <div style={{ padding: 12 }}>
                        <div style={{ marginBottom: 6 }}>
                          <strong>Niveau / Rôle</strong>
                          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 6 }}>
                            {typeof r.niveauOrdre === "number" && <Tag>Ordre: {r.niveauOrdre}</Tag>}
                            {r.niveauNom && <Tag color="blue">{r.niveauNom}</Tag>}
                            {r.roleRequis && <Tag color="geekblue">{r.roleRequis}</Tag>}
                          </div>
                        </div>
                        ...
                        <div style={{ marginBottom: 6 }}>
                          ...
                          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 6 }}>
                            <span>{statutTag(r.statutAvant)}</span>
                            <span style={{ color: "#999" }}>→</span>
                            <span>{statutTag(r.statutApres)}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ marginBottom: 6 }}>
                          ...
                          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 6 }}>
                            <Tag>{(getL(r).nom) || "-"}</Tag>
                            {typeof getL(r).prix === "number" && <Tag>Prix: {getL(r).prix}</Tag>}
                            {(typeof getL(r).latitude === "number" && typeof getL(r).longitude === "number") && (
                              <Tag>Coord: {getL(r).latitude}, {getL(r).longitude}</Tag>
                            )}
                          </div>
                        </div>
...
const DetailContent = ({ rec }) => {
  ...
  return (
    <div style={{ paddingRight: 4 }}>
      ...
      <Title level={5} className="!mt-0">Niveau / Rôle</Title>
      <Space size={[8, 8]} wrap>
        {typeof rec.niveauOrdre === "number" && <Tag>Ordre: {rec.niveauOrdre}</Tag>}
        {rec.niveauNom && <Tag color="blue">{rec.niveauNom}</Tag>}
        {rec.roleRequis && <Tag color="geekblue">{rec.roleRequis}</Tag>}
      </Space>

      <Divider />

      <Title level={5} className="!mt-0">Transition de statut</Title>
      <Space size={[8, 8]} wrap>
        {statutTag(rec.statutAvant)}
        <span style={{ color: "#999" }}>→</span>
        {statutTag(rec.statutApres)}
      </Space>
